...
from utils import create_session_code

def session_creator_code_menu():
    while True:
        ...
        if subchoice == "1":
                ...
                try:
                    ...
                    if 1 <= device_num <= len(devices):
                        ...
                        if 'CODE' not in config:
                            config['CODE'] = {}
                        config['CODE']['device_id'] = str(device_num - 1)
            ...
            try:
                ...
                config['CODE']['api_id'] = api_id
                config['CODE']['api_hash'] = api_hash
            ...
            asyncio.run(create_session_code())
            Write.Print("\n    Нажмите Enter для продолжения...", Colors.blue_to_cyan, interval=0.0005)
            input()
        elif subchoice == "4":
            try:
                folder_path = os.path.abspath('results/create_session_code/sessios')
