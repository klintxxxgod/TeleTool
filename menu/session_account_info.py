...
from utils import account_info_save_info
from pystyle import Write, Colors, Colorate
from menu import banner, print_separator
    
def session_account_info_menu():
    while True:
        ...
        Write.Print("    ┃ 5 ┃ Начать анализ аккаунта\n", Colors.blue_to_cyan, interval=0.0005)
        ...
        if subchoice == "1":
                ...
                try:
                    ...
                    if 1 <= device_num <= len(devices):
                        ...
                        if 'CHECK' not in config:
                            config['CHECK'] = {}
                        config['CHECK']['device_id'] = str(device_num - 1)
            ...
            try:
                ...
                config['CHECK']['api_id'] = api_id
                config['CHECK']['api_hash'] = api_hash
            ...
            try:
                folder_path = os.path.abspath('results/account_info_save/sessions')
            ...
            try:
                folder_path = os.path.abspath('results/account_info_save/dumps')
                os.makedirs(folder_path, exist_ok=True)
                os.startfile(folder_path)
            except Exception as e:
                Write.Print("\n    Ошибка при открытии директории!\n", Colors.blue_to_cyan, interval=0.0005)
            ...
            result = asyncio.run(account_info_save_info())
            if result:
                Write.Print(f"\n    [+] Анализ успешно завершен\n    [+] Результаты сохранены в папке results/account_info_save/dumps\n", Colors.cyan_to_blue, interval=0.0005)
            else:
